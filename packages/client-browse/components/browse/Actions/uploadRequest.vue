<template>
    ...
    <!-- <SvgIcon src="/icons/file/uploadRequest.svg" round content="upload request"
        ...
        <el-dialog v-model="dialogOpened" class="scroll-dialog" append-to-body
            :close-on-click-modal="false"
            :title="$t('publicUpload_requestFileUpload')"
            :visible.sync="popoverShow"
            >
            <FromRenderer ref="FromRendererRef" :form-json="formJson"/>
            <template #footer>
              <el-button :loading="loading" type="primary" @click="handleSubmit">{{ $t('confirm') }}</el-button>
            </template>
        </el-dialog>
    </div>
</template>
...
// #region module:
  ...
  async function handleSubmit () {
    const data = await FromRendererRef.value.vFormRenderRef.getFormData().then(res => {
      return res
    }).catch(error => {
      return false
    })
