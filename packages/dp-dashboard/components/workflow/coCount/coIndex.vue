...
function getCoLayout(st) {
    if(!st.layout) st.layout = []
    return st.displayList.reduce((prev,item, index) => {
        let layoutItem =  st.layout.find(lItem => lItem.component === item)
        if(!layoutItem) {
            layoutItem = {
                minW: 2,
                minH: 2,
                ...
                w: 2,
                h: 2,
                x: ((st.layout.length + index + 1) * 2) % 4,
                y: st.layout.length +  4 + index + 1, // puts it at the bottom
                i: new Date().valueOf() + index + 1,
                component: item,
                setting: st
            }
        } else {
            layoutItem.setting = { ...layoutItem.setting, ...st }
        }
...
watch(() => props.setting, (newSetting) => {
    if(newSetting && newSetting.displayList) state.layout = getCoLayout(deepCopy(newSetting))
}, {
    immediate: true
})
defineExpose({
