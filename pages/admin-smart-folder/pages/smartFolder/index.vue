<template>
    <NuxtLayout class="fit-height withPadding">
        ...
        <SmartFolderInfoDialog ref="SmartFolderInfoDialogRef" @refresh="handlePaginationChange(1)"/>
    </NuxtLayout>
</template>
...
// #region module: page
    ...
    const tableKey = TABLE.ADMIN_SMART_FOLDER
    ...
    async function getList (param) {
        state.loading = true
        try {
            const res = await GetSFolderPageApi({ ...param, ...state.extraParams })
...
async function handleDelete(id: string) {
    ...
    await DeletePreSearchApi(id)
    handlePaginationChange(pageParams.pageNum + 1)
}
function handleDblclick(row) {
    router.push(`/smartFolder/${row.id}`)
}
...
// #region module: ResponsiveFilterRef
    ...
    async function getFilter() {
        const filters = await GetSFolderPageConditionApi()
